...
The capsule is set up for development first: it's easy to burn through premium requests while just testing your data, utility functions, prompts, skills etc. so the default model is set to the cheap and cheerful `claude-haiku-4.5` (0.33x at the time of writing). Just keep in mind that code generation, tool calling, context window and basic ability to follow instructions will be significantly worse than flagship models like`claude-sonnet-4.6` or `gpt-5.5`.
